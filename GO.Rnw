...
\begin{figure}
  \centering
  \includegraphics[width=0.6\linewidth]{gaussianresponse}
...
gives the number of desired parallel processes). Function \code{GO2}
...
very slow, and therefore we use only \code{GO2}. Let us first evaluate
models for one to three dimensions:
<<>>=
data(varespec)
system.time(m1 <- GO2(varespec, k=1, freqlim=10, tot=100, iterlim=1000))
system.time(m2 <- GO2(varespec, k=2, freqlim=10, tot=100, iterlim=1000))
system.time(m3 <- GO2(varespec, k=3, freqlim=10, tot=100, iterlim=1000))
@ 
...
\begin{figure}
  \subfigure[a]{\includegraphics[width=0.5\linewidth]{GO-plot-GO2}}%
  ...
  \caption{One-dimensional Gaussian Ordination. \textbf{a} Dedicated
    \code{plot} function of \code{GO2} showing fitted Gaussian
  ...
  \label{fig:plotm1}
\end{figure}
<<plot-GO2,fig=true,include=false>>=
plot(m1, label = TRUE)
@ 
...
\code{ordiplot} function that is able to handle \code{GO2} results
(Fig.~\ref{fig:plotm1}).
...
and \code{scores} are can handle \code{GO2} results.
...
## take the same subset of species as used in GO2
v10 <- varespec[, colSums(varespec>0) >= 10]
## the gradient
ax <- drop(scores(m1))
...
\begin{figure}
  ...
  \caption{\emph{Cladina stellaris}, the most influential species in the ordination example.}
  \label{fig:Claste}
\end{figure}
...
\emph{Cladina stellaris}, Fig.~\ref{fig:Claste}) contributes more than
...
data(dune)
mdu1 <- GO2(dune, k=1, iterlim=1000)
mdu1
...
but then light mindedly decided to also run a GO2 on the full
